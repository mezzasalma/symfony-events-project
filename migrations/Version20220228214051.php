 ...
 */
final class Version20220228214051 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__event AS SELECT id, name, start_date, city, postal_code, price, number_place, active, location, end_date, description FROM event');
        $this->addSql('DROP TABLE event');
        $this->addSql('CREATE TABLE event (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL, start_date DATETIME NOT NULL, city VARCHAR(100) NOT NULL, postal_code VARCHAR(5) NOT NULL, price NUMERIC(5, 2) NOT NULL, seats INTEGER NOT NULL, active BOOLEAN DEFAULT 0 NOT NULL, location VARCHAR(255) DEFAULT NULL, end_date DATETIME NOT NULL, description CLOB DEFAULT NULL)');
        $this->addSql('INSERT INTO event (id, name, start_date, city, postal_code, price, seats, active, location, end_date, description) SELECT id, name, start_date, city, postal_code, price, number_place, active, location, end_date, description FROM __temp__event');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__event AS SELECT id, name, start_date, end_date, location, city, postal_code, price, active, description, seats FROM event');
        $this->addSql('DROP TABLE event');
        $this->addSql('CREATE TABLE event (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL, start_date DATETIME NOT NULL, end_date DATETIME NOT NULL, location VARCHAR(255) DEFAULT NULL, city VARCHAR(100) NOT NULL, postal_code VARCHAR(5) NOT NULL, price NUMERIC(5, 2) NOT NULL, active BOOLEAN DEFAULT 0 NOT NULL, description CLOB DEFAULT NULL, number_place INTEGER NOT NULL)');
        $this->addSql('INSERT INTO event (id, name, start_date, end_date, location, city, postal_code, price, active, description, number_place) SELECT id, name, start_date, end_date, location, city, postal_code, price, active, description, seats FROM __temp__event');
        $this->addSql('DROP TABLE __temp__event');
        $this->addSql('DROP INDEX IDX_7CE748AA76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__reset_password_request AS SELECT id, user_id, selector, hashed_token, requested_at, expires_at FROM reset_password_request');
        $this->addSql('DROP TABLE reset_password_request');
